...
def _normalize_lang(code: str | None) -> str:
    ...
    if code in SUPPORTED_LANGS: return code
    return code[:2] if code[:2] in SUPPORTED_LANGS else "en"

def resolve_lang(data: dict) -> str:
    cand = (data.get("lang") or data.get("language") or data.get("locale") or data.get("accept_language") or "")
    if isinstance(cand, str) and "," in cand: cand = cand.split(",")[0]
...
def reason_to_tip_localized(reason: str, analysis: dict, lang: str) -> str:
    ...
    if "low focus" in r: return t["blur"]
    if "shoulders not fully visible" in r or "subject too far" in r: return t["shoulders"]
    if "camera tilt" in r: return t["tilt"]
    if "not facing camera enough for front view" in r: return t["front_face"]
    if "not turned enough for side view" in r: return t["side_turn"]
    if isinstance(dist, (int, float)):
        if dist < 1.2: return t["too_close"]
        if dist > 3.0: return t["too_far"]
    return t["generic"]
...
def tips_for_rejected_localized(entry: dict, lang: str) -> list[str]:
    ...
    if not tips: tips.append(TIPS_L10N.get(lang, TIPS_L10N["en"])["generic"])
    return tips

def _collect_role_labels(photos_dict) -> list[str | None]:
    if not isinstance(photos_dict, dict): return []
    labels = []
...
def _build_role_report(roles=("front","side","back"),
    ...
    accepted = accepted or []; rejected = rejected or []
    provided_labels = provided_labels or []; index_to_b64 = index_to_b64 or {}
    ...
    for i, label in enumerate(provided_labels):
        if label in roles: provided_idx_by_role[label].append(i)
    acc_idx = {e["index"] for e in accepted}; rej_idx = {e["index"] for e in rejected}
    report = {}
    for role in roles:
        chosen_b64 = chosen_by_role.get(role); chosen_idx = b64_to_index.get(chosen_b64, None)
        status = "missing"; reasons = []; tips = []; failed_indices = []
        if chosen_idx is not None:
            if chosen_idx in acc_idx: status = "ok"
            elif chosen_idx in rej_idx:
                status = "retry"
                r = next((e for e in rejected if e["index"] == chosen_idx), None)
                if r: reasons = r.get("reasons", [])[:]; tips = r.get("tips", [])[:]
            else:
                status = "retry"
        for idx in provided_idx_by_role.get(role, []):
            if idx in rej_idx: failed_indices.append(idx)
        report[role] = {
            "status": status,
...
def topk_by_role(analyses: List[Dict[str, Any]], photos_b64: List[str], k: int = 2) -> Dict[str, List[str]]:
    ...
    for role in ("front", "side", "back"):
        ...
        if role == "front": cands.sort(key=lambda t: (-t[1], t[2]))
        elif role == "side": cands.sort(key=lambda t: (-t[1], -t[2]))
...
def calibrate_input(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    shoulder = cm("shoulder"); inseam   = cm("inseam");  arm    = cm("arm")
    ...
    out: Dict[str, Any] = {
        "ok": True, "lang": lang, "height_m": height_m,
        "chest": chest, "waist": waist, "hips": hips,
        "shoulder": shoulder, "inseam": inseam, "arm": arm,
        "foot_length_m": foot_len, "foot_width_m": foot_wid, "foot_width_category": foot_cat,
    ...
    if isinstance(unordered, list) and unordered:
        ...
        for k in ("front", "side", "back"):
            ...
            if isinstance(v, str): b64_list.append(v)
    ...
    # 6) choose best per role; also build top-N per role
    if accepted:
    ...
    # 9) pose hint (from chosen front if available)
    pose_hint = {}
    if accepted:
        try:
            # pick best front from accepted
            fronts = [(e["index"], e["analysis"]) for e in accepted if e["analysis"].get("role") == "front"]
            if not fronts:
