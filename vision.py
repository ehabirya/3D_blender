# app/vision.py  shared utilities + photo analysis + quality gates
...
def _shoulders(pose_lm, w: int, h: int) -> Tuple[Optional[np.ndarray], Optional[float], Optional[float], Optional[np.ndarray]]:
    ...
    try:
        ...
        vec = pR - pL
        length = float(np.linalg.norm(vec))  # pixels
...
def _face_box_and_yaw(face_lm, w: int, h: int) -> Tuple[Optional[List[int]], Optional[float], Optional[float]]:
    ...
    try:
        ...
        yaw = float((nose.x - 0.5 * (left.x + right.x)) / denom)  # 0~front, +right/-left
        abs_yaw = abs(yaw)
    except Exception:
        pass
...
def _person_bbox_height_px(pose_lm, face_box, h: int) -> Optional[float]:
    if pose_lm:
        try:
            ...
            pass
    if face_box:
        # crude proxy when only face detected
...
DEFAULT_THRESHOLDS = {
    "min_focus": 200.0,                 # blur threshold (Laplacian variance)
    "min_shoulder_ratio": 0.20,         # shoulder_len_px / image_width
    "max_roll_deg": 10.0,               # camera roll (tilt)
    "front_max_abs_yaw": 0.20,          # to accept a frame as "front"
    "side_min_abs_yaw": 0.35,           # to accept a frame as "side"
}
...
def quality_ok(analysis: Dict[str, Any], role_hint: Optional[str], thr: Dict[str, float] = DEFAULT_THRESHOLDS) -> Tuple[bool, List[str]]:
    """Return (ok, reasons). role_hint can be 'front'|'side'|'back'|None."""
    reasons = []
    if analysis["focus"] < thr["min_focus"]:
        reasons.append(f"low focus ({analysis['focus']:.1f} < {thr['min_focus']})")
    if analysis["shoulder_len_ratio"] < thr["min_shoulder_ratio"]:
        reasons.append("shoulders not fully visible / subject too far")
    if abs(analysis["roll_deg"] or 0.0) > thr["max_roll_deg"]:
        reasons.append(f"camera tilt {analysis['roll_deg']:.1f}° > {thr['max_roll_deg']}°")

    if role_hint == "front" and (analysis["abs_yaw"] or 9.0) > thr["front_max_abs_yaw"]:
        reasons.append("not facing camera enough for front view")
    if role_hint == "side" and (analysis["abs_yaw"] or 0.0) < thr["side_min_abs_yaw"]:
...
def choose_roles(analyses: List[Dict[str, Any]], photos_b64: List[str]) -> Dict[str, Any]:
    """Pick best front/side/back; return by_role + per-photo scores (no re-encoding)."""
    # rank by (focus, shoulder ratio) and role-likeness via abs_yaw
    scored = []
    for idx, a in enumerate(analyses):
        q = a["focus"] * (1.0 + 0.4 * a["shoulder_len_ratio"])
        scored.append((idx, a["role"], q, a["abs_yaw"]))
    def best_for(role):
        cands = [(i, q, yaw) for (i, r, q, yaw) in scored if r == role]
        if not cands: cands = [(i, q, yaw) for (i, r, q, yaw) in scored]  # fallback
